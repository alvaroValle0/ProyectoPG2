...
// Script simple para insertar datos de tickets usando MySQLi
$host = 'localhost';
$dbname = 'gestion';
$username = 'root';
$password = '';
...
$mysqli->query("DELETE FROM tickets");
echo "✅ Tabla de tickets limpiada.\n";

// Verificar si existen reparaciones
$result = $mysqli->query("SELECT COUNT(*) as count FROM reparaciones");
$row = $result->fetch_assoc();
$repairCount = $row['count'];

if ($repairCount == 0) {
    echo "⚠️ No hay reparaciones en la base de datos. Creando datos de ejemplo...\n";
    
    // Crear usuario admin si no existe
    $mysqli->query("INSERT IGNORE INTO users (name, username, email, password, role, email_verified_at, created_at, updated_at) 
    ...
    $mysqli->query("INSERT IGNORE INTO tecnicos (user_id, especialidad, experiencia_anos, telefono, activo, created_at, updated_at) 
    ...
    foreach ($clientes as $cliente) {
        $stmt = $mysqli->prepare("INSERT IGNORE INTO clientes (nombres, apellidos, telefono, email, direccion, dpi, activo, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, NOW(), NOW())");
        $stmt->bind_param("ssssss", $cliente[0], $cliente[1], $cliente[2], $cliente[3], $cliente[4], $cliente[5]);
        $stmt->execute();
        $stmt->close();
    }
    ...
    foreach ($equipos as $equipo) {
        $stmt = $mysqli->prepare("INSERT IGNORE INTO equipos (cliente_id, numero_serie, marca, modelo, tipo_equipo, descripcion, cliente_nombre, cliente_telefono, cliente_email, costo_estimado, fecha_ingreso, estado, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 10) DAY), 'reparado', NOW(), NOW())");
        $stmt->bind_param("issssssssd", $equipo[0], $equipo[1], $equipo[2], $equipo[3], $equipo[4], $equipo[5], $equipo[6], $equipo[7], $equipo[8], $equipo[9]);
        $stmt->execute();
        $stmt->close();
    }
    ...
    foreach ($reparaciones as $reparacion) {
        $stmt = $mysqli->prepare("INSERT IGNORE INTO reparaciones (equipo_id, tecnico_id, descripcion_problema, diagnostico, solucion, costo, estado, fecha_inicio, fecha_fin, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 10) DAY), DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 5) DAY), NOW(), NOW())");
        $stmt->bind_param("iisssds", $reparacion[0], $reparacion[1], $reparacion[2], $reparacion[3], $reparacion[4], $reparacion[5], $reparacion[6]);
        $stmt->execute();
        $stmt->close();
    }
...
$result = $mysqli->query("SELECT r.id, r.equipo_id, r.costo, e.cliente_nombre, e.cliente_telefono, e.cliente_email, e.marca, e.modelo 
                         FROM reparaciones r 
                         JOIN equipos e ON r.equipo_id = e.id 
                         WHERE r.estado = 'completada'");
$reparaciones = $result->fetch_all(MYSQLI_ASSOC);
...
foreach ($reparaciones as $reparacion) {
    ...
    $observacionesCliente = 'Cliente satisfecho con el servicio recibido';
    $dpiQuienFirma = '1234567890123';
    $tiempoGarantia = rand(30, 90);
    $observacionesGenerales = 'Ticket generado automáticamente por el sistema';
    
    $stmt->bind_param("ssisssddsssssssiss", 
        $numeroTicket,
        $tipoTicket,
        $reparacion['id'],
